...
exports.sendValidationResults = async (req, res) => {
  try {
    const { recipient, subject, message, validFileName, invalidFileName } = req.body;
    ...
    if (req.files?.validFile) {
      const filename = validFileName || 'valid_data.xlsx';
      attachments.push({
        filename: filename,
    ...
    if (req.files?.invalidFile) {
      const filename = invalidFileName || 'invalid_data.xlsx';
      attachments.push({
        filename: filename,
